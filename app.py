...
def precipitation():
    ...
    all_precipication = []
    for date, prcp in data_last_year:
        if prcp != None:
            precip_dict = {}
...
def tobs():
    ...
    data_last_year = session.query(Measurement.date, Measurement.tobs).filter(Measurement.station == most_active_station_id).filter(Measurement.date >= last_year).all()

    session.close()

    # Convert the query results to a dictionary using date as the key and temperature as the value.
    all_temperatures = []
    for date, temp in data_last_year:
        if temp != None:
            temp_dict = {}
            temp_dict[date] = temp
